...
void distribute_matrix_rows(char **lmat)
{
	...
	if(my_rank == 0)
	{
		...
		{
			int row_disp = (int)floor((double)i / col_div);
	...
	else
	{
		for(j = 0; j < row_group_size; j++)
		{
			MPI_Recv(lmat[j], DIM_LEN, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &stat);
		}
	}
}
...
void distribute_matrix_cols(char **mat)
{
	...
	if(my_rank == 0)
	{
		for(i = 1; i < comm_size; i++)
		{
			...
			{
				MPI_Send(mat[col_disp * cols_group_size + j], DIM_LEN, MPI_CHAR, i, 0, MPI_COMM_WORLD);
			}
		}
	}
	else
	{
		for(j = 0; j < cols_group_size; j++)
		{
			MPI_Recv(mat[j], DIM_LEN, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &stat);
		}
	}
}
